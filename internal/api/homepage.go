						...
						if mediaData, ok := mediaRel["data"].(map[string]interface{}); ok {
							...
							if fileEntity, found := includedMap[mediaID]; found {
								...
								if uri, ok := fileAttrs["uri"].(map[string]interface{}); ok {
									...
									// FIX: Check if Drupal is already sending an absolute URL.
									// If it starts with http, it's Cloudflare R2; don't add baseURL.
									if len(rawURL) > 4 && rawURL[:4] == "http" {
										attrs["field_media"] = rawURL
									} else {
										// Otherwise, it's a relative path (common in local dev).
										attrs["field_media"] = baseURL + rawURL
									}
								}
							}
						}
