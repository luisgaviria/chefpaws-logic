...
import (
	...
	"fmt"
	"net/http"
	"os"      // Added for environment check
	"strings" // Added for URL string manipulation
            ...
            if mediaData, ok := mediaRel["data"].(map[string]interface{}); ok {
              ...
              if fileEntity, found := includedMap[mediaID]; found {
                ...
                if uri, ok := fileAttrs["uri"].(map[string]interface{}); ok {
                  rawURL := uri["url"].(string)

                  var finalURL string
                  // Check if Drupal is sending an absolute URL (Cloudflare R2)
                  if len(rawURL) > 4 && rawURL[:4] == "http" {
                    finalURL = rawURL
                  } else {
                    // Relative path (common in local dev)
                    finalURL = baseURL + rawURL
                  }

                  // --- LOCAL DEV FIX ---
                  // Inject 's3fs-public' into the URL only if on DDEV.
                  // This ensures Cloudflare R2 finds the file in the subfolder locally.
